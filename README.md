...
> **대회 출품용 시제품**이다(2026 원티드 AI Championship · 모두의창업 시즌2). 정식 서비스가 아니며, 페이지·문서에 적힌 숫자는 전부 우리가 직접 잰 값이다 — 재지 않은 숫자는 적지 않는다.
...
| `docs/` | 설계 메모 — 코드 주석의 「기획서 §n」「ADR-000n」이 가리키는 설계 문서 요약 (`docs/design-notes.md`) |
